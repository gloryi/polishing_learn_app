...
quadra_r = 0
quadra_phase = "INHALE"
def clip_color(_): return 0 if _ <= 0 else 255 if _ >= 255 else int(_)
def inter_color(v1, v2, p): return clip_color(v1 + (v2-v1)*p)


def interpolate(col1, col2, percent): return (inter_color(col1[0], col2[0], percent),
                                              inter_color(
    col1[1], col2[1], percent),
...
sixtlets = SixtletsProcessor(
    X, Y, pygame, display_surface, upper_stats, "latvian_words", TEST_LANG_DATA)
...
for time_delta in delta_timer:
    ...
    pygame.draw.circle(trans_surface,
                       interpolate(quadra_col_1, quadra_col_2,
                                   quadra_timer.get_percent()),
                       (X//2, action_trigger),
                       (X//2-100)*quadra_w_perce1+100)
    pygame.draw.circle(trans_surface,
                       interpolate(quadra_col_1, quadra_col_2,
                                   quadra_timer.get_percent()**2),
                       (X//2, action_trigger),
